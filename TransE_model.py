...
def plot_named_clusters_interactive(embeddings, labels, num_clusters=10, title="Clustered Entity Embeddings"):
    pca = PCA(n_components=2)
    reduced = pca.fit_transform(embeddings)
    kmeans = KMeans(n_clusters=num_clusters, random_state=0).fit(embeddings)
    clusters = kmeans.labels_

    # Extract top entity names for each cluster
    df = pd.DataFrame({
        'x': reduced[:, 0],
        'y': reduced[:, 1],
        'Entity': labels,
    ...
    # Define human-readable labels (replace with your own after reviewing)
    cluster_names = {
        0: "Named Roles / Historical & Cultural Professions",
        1: "Named Entities – Mixed (Places, People, Orgs)",
        2: "Natural & Industrial Entities",
        3: "Media & Entertainment People",
        4: "Ontological Concepts and Roles",
        5: "Technology, History & Bio Entities",
        6: "Scientific & Cultural Categories",
        7: "Geographical & Sports Ontologies",
        8: "Multimedia Resources (Images / Files)",
        9: "Miscellaneous Notables"
    }

    df['Cluster Label'] = df['Cluster'].map(cluster_names)

    fig = px.scatter(
        df, x='x', y='y', color='Cluster Label',
    ...
    # Optionally preview samples per cluster
    for label in cluster_names.values():
        print(f"\n Sample from cluster: {label}")
    print(df[df['Cluster Label'] == label]['Entity'].head(5).to_list())
...
if __name__ == "__main__":
    ...
    try:
        ...
        print("Saved entity embeddings to", os.path.abspath("entity_embeddings.csv"))
    except Exception as e:
        print("Error saving entity embeddings:", e)
    ...
    try:
        ...
        print("Saved relation embeddings to", os.path.abspath("relation_embeddings.csv"))
    except Exception as e:
        print("Error saving relation embeddings:", e)
    relation_df.to_csv("relation_embeddings.csv")
    ...
    plot_named_clusters_interactive(
